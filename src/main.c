...
vector3_t vector3_negate(vector3_t v) { return vector3_scale(v, -1.0); }

double vector3_dot(vector3_t v1, vector3_t v2) {
    return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
}

double vector3_length(vector3_t v) { return sqrt(vector3_dot(v, v)); }

vector3_t vector3_normalize(vector3_t v) { return vector3_scale(v, 1 / vector3_length(v)); }
...
vector3_t vector3_transform(vector3_t v, matrix3_t transform) {
    return (vector3_t){ .x = transform.a11 * v.x + transform.a12 * v.y + transform.a13 * v.z,
...
static sphere_t spheres[] = { [0] = { .center = { 0, -1, 3 },
                                      .radius = 1,
                                      .color = { 1, 0, 0 },
                                      .specular = 500,
                                      .reflective = 0.2f },
...
double compute_lighting(
    vector3_t contact_point, vector3_t normal, vector3_t contact_point_to_camera,
        ...
        if (light->type == LIGHT_TYPE_AMBIENT) {
            ...
            if (dot > 0) {
                intensity += light->intensity * dot /
                             (vector3_length(light_vector) * vector3_length(normal));
            }
            ...
            if (dot2 > 0) {
                intensity +=
                    light->intensity * pow(dot2 / (vector3_length(reflection) *
                                                   vector3_length(contact_point_to_camera)),
                                           specular_exponent);
            }
        }
    }
...
color_t trace_ray(ray_t ray, double t_min, double t_max, int depth) {
    ...
    vector3_t contact_point =
        vector3_add(ray.position, vector3_scale(ray.direction, intersection.t));
    vector3_t normal =
        vector3_normalize(vector3_subtract(contact_point, intersection.sphere->center));

    double lighting = compute_lighting(
        contact_point, normal, vector3_negate(ray.direction), intersection.sphere->specular
    );
    color_t local_color = { .r = clamp((intersection.sphere->color.r * lighting), 0, 1),
    ...
    color_t reflected_color = trace_ray(
        (ray_t){ .position = contact_point, .direction = reflected_direction },
        0.001,
        INFINITY,
        depth - 1
    );

    return (color_t){ .r = lerp(local_color.r, reflected_color.r, r),
...
vector3_t canvas_to_viewport(int canvas_x, int canvas_y) {
    return (vector3_t){ .x = (double)canvas_x * VIEWPORT_WIDTH / CANVAS_WIDTH,
...
vector2_t canvas_to_screen(int canvas_x, int canvas_y) {
    return (vector2_t){ .x = CANVAS_WIDTH / 2.0 + (double)canvas_x,
...
void put_pixel(int canvas_x, int canvas_y, color_t color) {
    ...
    DrawPixel(
        (int)screen_coord.x,
        (int)screen_coord.y,
        (Color){ .r = (uint8_t)round(color.r * 255),
...
int main() {
    ...
    camera_t camera = { .position = { 0, 0, 0 },
                        .rotation = matrix3_multiply(
                            matrix3_y_rotation_from_angle(0), matrix3_x_rotation_from_angle(0)
                        ) };
    ...
    for (int x = -CANVAS_WIDTH / 2; x <= CANVAS_WIDTH / 2; ++x) {
        for (int y = -CANVAS_HEIGHT / 2; y <= CANVAS_HEIGHT / 2; ++y) {
            ray_t ray = { .position = camera.position,
                          .direction =
                              vector3_transform(canvas_to_viewport(x, y), camera.rotation) };
